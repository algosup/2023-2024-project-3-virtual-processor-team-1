...
void readFile(char *filename, char *buffer, int buffer_size)
{
...
void cleanFile (char *filename, char *buffer, int buffer_size)
{
    // open the file
    FILE *file = fopen(filename, "r");
    ...
    snprintf(newFileName, sizeof(newFileName), "%.*sClean%s", (int)(dot - filename), file, dot);
